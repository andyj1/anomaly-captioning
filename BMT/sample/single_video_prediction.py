...
if __name__ == "__main__":
    ...
    parser.add_argument('--duration_in_secs', type=float, required=False)
    ...
    args.duration_in_secs = float(subprocess.check_output(f"ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 sample_output/original/{args.video_name}.mp4", shell=True))
    print("duration:", args.duration_in_secs)
    ...
    # save json as args.video_name
    os.makedirs(f'./sample_output/captions/', exist_ok=True, mode=0x755)
    json.dump(captions, open(f'./sample_output/captions/{args.video_name}.json', 'w'), indent=6)
    print(captions)
